...
export const Events = {
  ...
  async create(ctx, payload) {
    const { title, description, date, location_id, capacity } = payload;
    ...
    const { rows } = await executeQuery("Events", "create", [title, description, date, location_id, capacity, created_by]);
    return { ok: true, data: rows[0] };
  },

  async update(ctx, payload) {
    const { id, title, description, date, location_id, capacity } = payload;
    const { rows } = await executeQuery("Events", "update", [id, title, description, date, location_id, capacity]);
    if (rows.length === 0) return { ok: false, code: "NOT_FOUND", error: "Event not found" };
    return { ok: true, data: rows[0] };
  },
